...
func Redeem(key string, userId int) (result *RedeemResult, err error) {
	...
	if result != nil && result.Type == RedemptionTypeSubscription {
		if upgradeGroup != "" {
			_ = UpdateUserGroupCache(userId, upgradeGroup)
		}
		RecordLog(userId, LogTypeTopup, fmt.Sprintf("通过兑换码兑换订阅 %s，订阅ID %d，兑换码ID %d", result.PlanTitle, result.SubscriptionId, redemption.Id))
		return result, nil
	}
